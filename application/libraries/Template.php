...
class Template
{
	...
	// --------------------------------------------------------------------
	/**
	 * Define o local onde os arquivos de visão devem estar
	 */
	public function set_view_home($view_dir){
	...
	{
		$controller_name = $this->_view_dir; 
		if(!$controller_name){
			$controller_name = strtolower($this->_ci->router->class);
		}
		...
		{ 
			...
			$this->_title = APP_NAME." | ".$this->_guess_title();
		}

		/**
		 * reverte os array's para respeitar a ordem do arquivo de configuração
		 */
		// default js 
		foreach (  array_reverse($this->_default_js) as $js_entry)
		{
			$this->js($js_entry,'',true);
		}
		// default data
		foreach (($this->_default_css) as $css_entry)
		{
			$this->css($css_entry,'',true);
		}
		
		$this->_metadata = array_merge($this->_metadata, $this->_css, $this->_js);
		...
		if ($this->_layout)
		{
			...
			$this->_body =  self::_load_view('templates/'.$this->_layout, $this->_data, TRUE, self::_find_view_folder());
		}
	...
	/**
	 * Return set layout or FALSE if it hasnt been set yet.
	 */
	...
	public function get_layouts()
	{
		...
		foreach(glob(self::_find_view_folder().'templates/*.*') as $layout)
		{
	...
	public function get_theme_layouts($theme = NULL)
	{
		...
		foreach ($this->_theme_locations as $location)
		{
			...
			if(is_dir($location.$theme.'/views/templates/'))
			{
				foreach(glob($location.$theme . '/views/templates/*.*') as $layout)
				{
					$layouts[] = pathinfo($layout, PATHINFO_BASENAME);
				}
				break;
	 ...
	 * @access	public
	 */
	public function css($file, $subdir="assets/css/",$preped=false)
	{
		if(!$this->endsWith($subdir,"/")){
			$subdir .="/";
		}
		$url = base_url($subdir.$file);
		$file= "<link rel='stylesheet' href='$url' type='text/css' ></link>";
		if($preped){
			array_unshift($this->_css, $file);
		}else{
	 ...
	 * @access	public
	 */
	public function js($file, $subdir="assets/js/",$preped=false)
	{
		if(!$this->endsWith($subdir,"/")){
			$subdir .="/";
		}
		$url = base_url($subdir.$file);
		$file= "<script type='text/javascript' src='$url' ></script>";
		if($preped){
			array_unshift($this->_js, $file);
		}else{
	...
	public function layout_exists($layout)
	{
		...
		return file_exists(self::_find_view_folder().'templates/' . $layout . self::_ext($layout));
	}
